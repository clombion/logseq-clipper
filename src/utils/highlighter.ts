...
function updateUndoRedoButtons() {
	const undoButton = document.getElementById('logseq-undo-highlights');
	const redoButton = document.getElementById('logseq-redo-highlights');
...
export function createHighlighterMenu() {
	...
	let menu = document.querySelector('.logseq-highlighter-menu');

	// If the menu doesn't exist, create it
	if (!menu) {
		menu = document.createElement('div');
		menu.className = 'logseq-highlighter-menu';
	...
	if (highlightCount > 0) {
		...
		clipButton.id = 'logseq-clip-button';
	...
	undoButton.id = 'logseq-undo-highlights';
	...
	redoButton.id = 'logseq-redo-highlights';
	...
	exitButton.id = 'logseq-exit-highlighter';
	...
	if (highlightCount > 0) {
		...
		const clearButtonEl = menu.querySelector('#logseq-clear-highlights') as HTMLButtonElement;
		const clipButtonEl = menu.querySelector('#logseq-clip-button') as HTMLButtonElement;
	...
	const exitButtonEl = menu.querySelector('#logseq-exit-highlighter') as HTMLButtonElement;
	const undoButtonEl = menu.querySelector('#logseq-undo-highlights') as HTMLButtonElement;
	const redoButtonEl = menu.querySelector('#logseq-redo-highlights') as HTMLButtonElement;
...
function removeHighlighterMenu() {
	const menu = document.querySelector('.logseq-highlighter-menu');
	if (menu) {
		menu.remove();
	}
}
...
export async function loadHighlights() {
	...
	if (storedData && Array.isArray(storedData.highlights) && storedData.highlights.length > 0) {
		...
		if (generalSettings.alwaysShowHighlights) {
			applyHighlights();
			document.body.classList.add('logseq-highlighter-always-show');
		}
	} else {
		highlights = [];
	}
...
function handleKeyDown(event: KeyboardEvent) {
	if (event.key === 'Escape' && document.body.classList.contains('logseq-highlighter-active')) {
		exitHighlighterMode();
