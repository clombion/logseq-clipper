...
export const fragment_link = (str: string, param?: string): string[] => {
	...
	const linktext = String(match?.[1]?.trim().replace(/(['"])/g, '') || 'link'); // Custom name (if any), with quotes removed
	const currentUrl = String(match?.[2] || param); // URL (or whole param if no URL found)

	const extractTextFragmentParts = (selectedText: SelectedText): TextFragmentParts => {
		...
		if (Array.isArray(data)) {
			return data.map((item) => {
				if (typeof item === 'object' && item !== null && 'text' in item) {
					return {
						...item,
						text: `${item.text} [${linktext}](${currentUrl}${createTextFragmentUrl(item.text)})`,
					};
				} else {
					return `${item} [${linktext}](${currentUrl}${createTextFragmentUrl(String(item))})`;
				}
			});
			...
			return [`${data} [${linktext}](${currentUrl}${createTextFragmentUrl(data)})`];
		}
	} catch (error) {
